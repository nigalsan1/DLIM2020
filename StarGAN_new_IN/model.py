...
class Generator(nn.Module):
    ...
    def __init__(self, conv_dim=64, c_dim=5, repeat_num=6):
        ...
        self.L2 = ccbn() #Input & Output Size?
        self.L3 = nn.ReLU(inplace=True)

        # Down-sampling layers.
        curr_dim = conv_dim
        self.L4 = nn.Conv2d(curr_dim, curr_dim*2, kernel_size=4, stride=2, padding=1, bias=False)
        self.L5 = ccbn() #Input & Output Size?
        self.L6 = nn.ReLU(inplace=True)

        curr_dim = curr_dim * 2

        self.L7 = nn.Conv2d(curr_dim, curr_dim*2, kernel_size=4, stride=2, padding=1, bias=False)
        self.L8 = ccbn() #Input & Output Size?
        self.L9 = nn.ReLU(inplace=True)

        #Bottleneck Layers
        self.L10 = ResidualBlock(dim_in=curr_dim, dim_out=curr_dim)
        self.L11 = ResidualBlock(dim_in=curr_dim, dim_out=curr_dim)
        self.L12 = ResidualBlock(dim_in=curr_dim, dim_out=curr_dim)
        self.L13 = ResidualBlock(dim_in=curr_dim, dim_out=curr_dim)
        self.L14 = ResidualBlock(dim_in=curr_dim, dim_out=curr_dim)
        self.L15 = ResidualBlock(dim_in=curr_dim, dim_out=curr_dim)

        # Up-sampling layers.
        self.L16 = nn.ConvTranspose2d(curr_dim, curr_dim//2, kernel_size=4, stride=2, padding=1, bias=False)
        self.L17 = ccbn() #Input & Output Size?
        self.L18 = nn.ReLU(inplace=True)

        curr_dim = curr_dim // 2

        self.L19 = nn.ConvTranspose2d(curr_dim, curr_dim//2, kernel_size=4, stride=2, padding=1, bias=False)
        self.L20 = ccbn() #Input & Output Size?
