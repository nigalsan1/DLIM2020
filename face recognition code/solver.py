...
class Solver(object):
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            one_hot_encoding = torch.zeros((1,5))
            one_hot_encoding[0][3] = 1
            
            
            i=0
            while True:
                i=i+1
            #for i, (x_real, c_org) in enumerate(data_loader):# x_real are the inputs to the stargan?
                # i dont need c_org cause im defining my own label , so to speak hardcoding it
                #x_real is the output of the camera of the camera
                
                ret,x_real = cap.read() 

                if ret == True:
                    #x_real = cv2.cvtColor(x_real,cv2.COLOR_BGR2RGB)
                    
                    # Convert to grayscale
                    color = cv2.cvtColor(x_real, cv2.COLOR_BGR2RGB)
                    # Detect the faces
                    ...
                    if len(faces)!=0 : 
                        for (x, y, w, h) in faces:
                            ...
                            if (x-30>0 and y-40>0) :
                                ...
                                face_crop.append(color[y-40:y+w+30,x-30:x+w+30])

                            else:
                                face_crop.append(color[:,:])
                    ...
                    for face  in face_crop:
                        ...
                        print(type(face))
                        #face.astype(np.uint16)
                        face_tensor = torch.from_numpy(face).permute(2,0,1)
                        print(face_tensor.type())
                        
                        #face_tensor = face_tensor.view(1, -1)
                        
                         
                        face_tensor = face_tensor.unsqueeze(0)
                        print(face_tensor.size(),' ',one_hot_encoding.size())
                        face_tensor = face_tensor.type(torch.ShortTensor)
                        print(face_tensor.type())
                        ...
                        #c_trg_list = one_hot_encoding
                        
                        x_fake_list = [face_tensor] # output of the network
                        #x_fake_list[0].IntTensor()
                        #print(x_fake_list[0].type())
                        
                        x_fake_list.append(self.G(face_tensor, one_hot_encoding))
                    ...
                    #out.write(x_real)
                    #get face open cv
                    # center crop face around 
                    #resize face opencv 128*128
                    #convert to pytorch
                    #normalize
                    #add new axis /x = x.view(1, -1)/x2 = x1.unsqueeze(0)
                    '''
                    x_real = x_real[np.newaxis,:]
                    
                    
                    x_real_tensor = torch.from_numpy(x_real) 
                    x_real_tensor = x_real.to(self.device)

                # Prepare input images and target domain labels.
                ### input images? what is c_trg_list
                #c_trg_list = self.create_labels(c_org, self.c_dim, self.dataset, self.selected_attrs)
                #onehotencoding 1x40
               
                    c_trg_list = one_hot_encoding
                # Translate images.
                    x_fake_list = [x_real_tensor] # output of the network
                
                    x_fake_list.append(self.G(x_real_tensor, one_hot_encoding))
